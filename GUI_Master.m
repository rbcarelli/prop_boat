...
panel = uipanel('Position',[.001 .001 .1 .35]);

btn_configure_daq = uicontrol(panel,'Style','togglebutton');
btn_configure_daq.String = 'Configure DAQ';
btn_configure_daq.Callback = @f_configure_daq;
set(btn_configure_daq,'Position',[10 180 100 25])

btn_acquire_enable = uicontrol(panel,'Style','togglebutton');
btn_acquire_enable.String = 'Start/Stop';
btn_acquire_enable.Callback = @f_control_acquire;
set(btn_acquire_enable,'Position',[10 150 100 25])

btn_trim_data = uicontrol(panel,'Style','pushbutton');
btn_trim_data.String = 'Trim Data';
btn_trim_data.Callback = @f_trim_data;
set(btn_trim_data,'Position',[10 120 100 25])

btn_config_serial = uicontrol(panel,'Style','pushbutton');
btn_config_serial.String = 'Open Port';
btn_config_serial.Callback = @f_serial_port_open;
set(btn_config_serial,'Position',[10 90 100 25])

btn_reset = uicontrol(panel,'Style','pushbutton');
btn_reset.String = 'Reset';
btn_reset.Callback = @f_reset;
set(btn_reset,'Position',[10 60 100 25])

btn_operate = uicontrol(panel,'Style','pushbutton');
btn_operate.String = 'Operate';
btn_operate.Callback = @f_operate;
set(btn_operate,'Position',[10 30 100 25])
